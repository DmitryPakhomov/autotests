...
class TestPingdom:
    ...
    def test_pingdom(self, browser: WebDriver, page: str, country: str):
        ...
        if page == '':
            ...
            active_session_loc = (
                By.XPATH,
                "//h6[contains(text(), 'Active Sessions (Now)')]//following-sibling::span"
            )
            load_time_loc = (
                By.XPATH, "//h6[contains(text(), 'Load Time (Now)')]//following-sibling::span")
            avg_load_time_loc = (
                By.XPATH,
                "//div[contains(text(), 'Avg. load time')]"
                "//ancestor::thead//following-sibling::tbody//td[3]//div//div"
            )
